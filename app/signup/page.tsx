...
function SignupPageContent() {
  ...
  const validate = () => {
    ...
    else if (!allCriteriaMet) newErrors.password = "Password does not meet all requirements.";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (success) {
      ...
      if (isRateLimit) {
        addToast({ title: "Too many attempts", description: "Email sending limit reached. Please wait a few minutes.", type: "error" });
      } else {
        setErrors({ email: SIGNUP.emailExists });
      }
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100 px-4 py-8">
      <Card className="w-full max-w-md rounded-2xl shadow-lg">
        ...
        <CardContent className="space-y-4">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                className={err("name") ? "border-red-500 focus-visible:ring-red-500" : ""}
              />
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                className={err("email") ? "border-red-500 focus-visible:ring-red-500" : ""}
              />
            ...
            <div className="space-y-1">
              ...
              <div className="relative">
                <Input
                  ...
                  className={`pr-10 ${err("password") ? "border-red-500 focus-visible:ring-red-500" : ""}`}
                />
              ...
              {formData.password.length > 0 && (
                <div className="space-y-2 mt-1">
                  ...
                  <div className="flex gap-1">
                    {[1, 2, 3, 4, 5].map((i) => (
                      <div key={i} className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${i <= strength.score ? strength.barColor : "bg-gray-200"}`} />
                    ))}
                  </div>
                </div>
              )}
              ...
              {(showPasswordCriteria || formData.password.length > 0) && (
                <div className="bg-gray-50 rounded-lg p-3 space-y-1.5 border border-gray-200 mt-1">
                  <p className="text-xs font-medium text-gray-600 mb-2">Password must contain:</p>
                  {criteria.map((c) => (
                    <div key={c.label} className="flex items-center gap-2">
                      {c.met ? <Check className="h-3.5 w-3.5 text-green-500 shrink-0" /> : <X className="h-3.5 w-3.5 text-red-400 shrink-0" />}
                      <span className={`text-xs ${c.met ? "text-green-700" : "text-gray-500"}`}>{c.label}</span>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="space-y-1">
              ...
              <div className="relative">
                <Input
                  ...
                  className={`pr-10 ${err("confirmPassword") ? "border-red-500 focus-visible:ring-red-500" : ""}`}
                />
              ...
              {err("confirmPassword") && <p className="text-xs text-red-500">{err("confirmPassword")}</p>}
              {!err("confirmPassword") && formData.confirmPassword && formData.password === formData.confirmPassword && (
          ...
          <Button variant="outline" className="w-full" onClick={handleGoogleSignup} loading={state.isLoading} disabled={state.isLoading}>
