  ...
  export default function ProductDetails({ product }: ProductDetailsProps) {
    ...
    // null = not yet fetched, {} = fetched but no inventory configured for this color
    ...
    useEffect(() => {
      ...
      setSizeStockMap(null); // reset while loading so sizes are blocked until data arrives
      fetch(
  ...
  if (rows.length === 0) {
    // Empty sizes array → all sizes are out of stock for this color
    ...
    return;
          ...
          setSizeStockMap(map);
          // If selected size is now out-of-stock or not in inventory, clear it
          setSelectedSize(prev => {
            if (!prev) return prev;
            const stockVal = map[prev];
            // stockVal undefined = size not in inventory = out of stock
            if (stockVal === undefined || stockVal <= 0) return "";
            return prev;
          });
        })
        .catch(() => setSizeStockMap({})) // on error, allow all sizes
        .finally(() => setSizesLoading(false));
    }, [selectedVariant, product.category]);

    /**
     * Determine if a size is disabled for the current color:
     * - While loading (sizeStockMap === null): disable all sizes
     * - After load with no inventory config (empty map {}): all sizes available
     * - After load with inventory entries: disable if size is missing from map OR stock <= 0
     */
    const isSizeDisabled = (size: string): boolean => {
      if (sizeStockMap === null) return true;           // still loading
      if (Object.keys(sizeStockMap).length === 0) return false; // no inventory = all OK
      const stock = sizeStockMap[size];
      return stock === undefined || stock <= 0;         // not in inventory or zero stock
    };

    // Whether any size is actually available to pick
    ...
    // Use color-level stock when available, otherwise fall back to product stock
    const effectiveStock = colorStock !== null ? colorStock : (product.stock ?? undefined);

    // Out-of-stock: either all sizes gone for this color, or the specific selected size is 0
    ...
    const handleAddToCart = () => {
      ...
      // Guard: prevent adding if the selected size is actually out of stock in inventory
    ...
    const handleBuyNow = () => {
      ...
      // Guard: same inventory check for Buy Now
      if (isSizeDisabled(selectedSize)) {
        addToast({
          title: "Size unavailable",
          description: `${selectedSize} is out of stock for this color. Please choose another size.`,
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                {/* All sizes always shown; loading skeleton while fetching, then crossed out when out of stock */}
                {sizeStockMap === null && sizesLoading ? (
                  /* Loading skeleton — sizes not yet determined, show placeholders */
                ...
                {/* "Out of Stock for this color" banner — shown when ALL sizes are 0 */}
                {allSizesOutOfStock && (
